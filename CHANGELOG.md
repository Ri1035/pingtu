...
### Added
- **文字斜切（skew）**：文字面板新增「斜切」滑条（-45°~45°，以文字中心为倾斜轴），让文字呈斜体轮廓；主拼图与长拼图同时生效，与缩放 / 旋转 / 描边 / 阴影正交叠加。
- **变形预设（一键套用）**：文字面板新增 6 个快捷变形按钮——常规 / 拉宽 / 拉高 / 压扁 / 左倾 / 右倾，点击即应用对应的水平 / 垂直缩放 + 斜切组合，便于快速塑造标题字型。
- **素材插入长图指定位置**：长拼图堆叠列表每一项可点击设为「插入锚点」，此时从素材库加号添加的新图会插入到该锚点图之后（而非追加末尾），便于把素材放进详情页中部；可随时取消锚点。

### Changed
- **长拼图渲染性能优化**：`drawLongCollage` 拆分 base / overlay 两级图层——背景 / 棋盘格 / 图片堆叠 / 标题并入 base（静态，缓存到离屏画布），文字、水印为 overlay（动态层）；拖拽文字时仅重画 overlay，避免逐帧重跑图片交叉渐变。
- **拖拽节流（rAF）**：长拼图与主拼图的文字拖拽改为 requestAnimationFrame 合并位移，一帧只提交一次状态更新，消除高频 pointer 事件导致的卡顿与抖动。
- `TextItem` 新增可选 `skewX`；`render.drawText` 与命中测试 `textBounds` 均计入斜切，保证拖拽命中与绘制一致。
- 版本号从 `v1.8.0` 升级到 `v1.9.0`。

### Notes
- 斜切以文字中心为原点做 `ctx.transform(1, 0, tan(skew), 1, ...)`，与中心缩放共存时变换矩阵叠加。
- npm run typecheck / build 通过（gzip ≈ 95.8 KB）。
