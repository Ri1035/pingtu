...
export function useCollage() {
  ...
  const addText = useCallback((partial?: Partial<TextItem>): TextItem => {
    ...
    const item: TextItem = {
      ...
      lineHeight: partial?.lineHeight ?? DEFAULT_TEXT.lineHeight,
      letterSpacing: partial?.letterSpacing ?? DEFAULT_TEXT.letterSpacing,
      align: partial?.align ?? DEFAULT_TEXT.align,
      underline: partial?.underline ?? DEFAULT_TEXT.underline,
      strokeColor: partial?.strokeColor ?? DEFAULT_TEXT.strokeColor,
      strokeWidth: partial?.strokeWidth ?? DEFAULT_TEXT.strokeWidth,
      shadowColor: partial?.shadowColor ?? DEFAULT_TEXT.shadowColor,
      shadowBlur: partial?.shadowBlur ?? DEFAULT_TEXT.shadowBlur,
      shadowOffsetX: partial?.shadowOffsetX ?? DEFAULT_TEXT.shadowOffsetX,
      shadowOffsetY: partial?.shadowOffsetY ?? DEFAULT_TEXT.shadowOffsetY,
      opacity: partial?.opacity ?? DEFAULT_TEXT.opacity,
      x: partial?.x ?? 0.5,
      y: partial?.y ?? 0.5,
    }
