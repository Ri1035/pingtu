...
- **长拼图（v1.7.0，电商详情页/公众号长图）**：侧边栏顶部独立 Tab「长拼图」，完全独立于主拼图；图片按顺序纵向堆叠成超长图（宽度 720~4096 可选），交界「模糊边缘」用纵向 alpha 渐变做交叉过渡，文字/水印/素材库复用主拼图面板，超长整图按段切片导出；头像压缩至约 27KB
- **长拼图增强（v1.8.0）**：素材库可直接加进长图堆叠队列；文字可在画布点击/拖动定位（命中与绘制同口径含变形，选中虚线框）；新文字默认落在当前视图停靠段中间；文字新增水平/垂直变形（非等比缩放，40%~250%）
- **文字斜切 + 变形预设 + 长图性能优化（v1.9.0）**：文字面板新增「斜切」滑条（-45°~45°）与 6 个变形预设（常规/拉宽/拉高/压扁/左倾/右倾）；长拼图渲染拆分为 base/overlay 两级图层（静态基础层离屏缓存）+ 拖拽 rAF 节流，消除文字拖动卡顿；素材库新增「插入长图指定位置」（堆叠项可设插入锚点）
- **自适应框体调整（v1.10.0）**：选中格子的放大/缩小改为编辑网格 fr 权重——放大某格，同行右侧与同列下方的框体自动收缩重排，整张拼图无重叠、总尺寸不变（替换旧版「浮起覆盖邻居」效果），预览与导出一致
- **扩充布局模板库（v1.11.0）**：`official-layouts.ts` 146→**207** 套模板，1~16 张每个数量均补充新模板（1 张新增拉宽/拉长、2~16 张各新增 4 套），全部通过 `validateLayout` 校验
...
| Tag | 指向 | 内容 |
| --- | --- | --- |
| v1.0.0 | 86c5707 | 首发：布局/文字/字体/导出 |
| v1.1.0 | 5b6e5b1 | hover 修复 + 入口强化 + 素材库 + 修图编辑器 |
| v1.2.0 | 32ad70b | 无缝拼图 + 单图自由调整 |
| v1.3.0 | cd15fad | hover 边框修复 + UI 设计系统 v2 + 浮层素材/单格调整 + 清空素材库 + 版本号工作流 |
| v1.4.0 | 581e7e8 | 浮层素材可移动/缩放 + 文字更多样式（行距/字距/对齐/下划线/描边/阴影/透明度） |
| v1.5.0 | 86f44a5 | 浮层素材加边框 + 边框样式(实线/虚线/点线/双线) + 自定义画布大小 + 单格框体调整与层级浮起 |
| v1.6.0 | 3bf88fc | 读取本机字体(Local Font Access API) + 水印(文字/图片、5 套模板) + 开发者信息页(#about 分享链接) |
| v1.6.1 | 5c7351a | 样式/导出、文字/水印页签合并；开发者信息页移除技术栈、姓名 KOKA、头像改用 avatar.png |
| v1.7.0 | eaf90cc | 长拼图模块（纵向堆叠 + 模糊边缘 + 文字/水印复用 + 切片导出）+ 头像压缩优化 |
| v1.8.0 | 30dea1c | 长拼图可用素材库；文字画布拖拽 + 新文字落当前段 + 水平/垂直变形 |
| v1.9.0 | 65ce446 | 文字斜切 + 变形预设 + 长图 base/overlay 图层缓存与拖拽节流 + 素材插入长图指定位置 |
| v1.10.0 | 66731e1 | 自适应框体调整（放大/缩小改编辑 fr 权重，其它框体自适应重排） |
| v1.11.0 | 089c15b | 扩充布局模板库（145→207 套，1~16 张每档均补充） |
