...
| 当前版本 | **v1.4.0**（package.json `"version": "1.4.0"`，tag `v1.4.0`） |
...
- **浮层素材（v1.3.0 + v1.4.0）**：素材库「添加为浮层」叠加到拼图上；v1.4.0 起支持拖拽移动 + 大小(5%~800%)/旋转/不透明度滑条 + 滚轮缩放
...
| v1.4.0 | 3c31a0f | 浮层素材可移动/缩放 + 文字更多样式（行距/字距/对齐/下划线/描边/阴影/透明度） |
